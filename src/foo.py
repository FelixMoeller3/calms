...
mas_strat = cl_strat.mas.MAS(model,optim,1.0,nn.CrossEntropyLoss(),[])
#ewc_strat = cl_strat.ElasticWeightConsolidation(model,optim,nn.CrossEntropyLoss())
...
mnist_first_dset_sizes = {'train': len(mnist_train_first), 'val': len(mnist_test_first)}
mas_strat.train(mnist_first_loaders, mnist_first_dset_sizes,5)
#ewc_strat.train(mnist_first_loaders,mnist_first_dset_sizes,5)
...
mas_strat.train(mnist_second_loaders, mnist_second_dset_sizes,5)
#ewc_strat.train(mnist_second_loaders, mnist_second_dset_sizes,10)
print('Testing first digits of MNIST after training all digits')
mas_strat._run_val_epoch(mnist_test_first_loader,len(mnist_test_first))
...
mas_strat._run_val_epoch(mnist_test_second_loader,len(mnist_test_second))
