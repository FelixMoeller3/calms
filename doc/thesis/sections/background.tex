...
Notable examples of structural approaches include Elastic Weight Consolidation (\gls{ewc}) \cite{kirkpatrick2017overcoming},Memory Aware
Synapses (\gls{mas}) \cite{aljundi2018memory}, Incremental Moment Matching (\gls{imm}) \cite{lee2017overcoming} as well as Asymmetric Loss
Approximation by Single-Side Overestimation (\gls{alasso}) \cite{park2019continual} which is an extension of Synaptic Intelligence (\gls{si})
...
Based Lifelong Learning (\gls{ebll}) is based on the approach of Li et al., however in \gls{ebll} the distillation loss is computed based on
