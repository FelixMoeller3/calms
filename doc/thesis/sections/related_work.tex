...
ALASSO is an extension of Synaptic Intelligence (SI) proposed by Zenke et al. \cite{zenke2017continual}
% Alpha equation
\begin{equation}
...
\begin{equation}
    L_s^n(\theta_k,a) = \begin{cases} \hat{\Omega} (\theta_k - \hat{\theta}_k)^2 & \text{if} \alpha(\theta_k) > 0 \\
    (a \hat{\Omega}^N_k + \epsilon)(\theta_k - \hat{\theta}_k)^2 & \text{if} \alpha(\theta_k) \leq 0 \end{cases}
\end{equation}
% Total loss equation
\begin{equation}
    \hat{L}^N = L^N + c \sum_k L_s^{N-1}(\theta_k,a)
\end{equation}
% Omega equation
\begin{equation}
