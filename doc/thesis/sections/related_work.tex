...
We present related work for this thesis in this chapter. First, we will outline the basic idea of each \gls{al} strategy we will
use in our experiments. Next, we give an overview of the \gls{cl} approaches we use. Finally, we will introduce ActiveThief, the
model stealing framework which we extend in this thesis.
...
CoreSet, sometimes also referred to as $k$-center strategy, is an active learning method specifically proposed for \glspl{cnn} \cite{sener2017active}.
Sener and Savarese redefine the pool-based active learning problem (for original definition see 
...
four times slower. We therefore omit the proposed algorithm in this section.
...
\begin{figure} [ht]
    ...
    \caption[Visualization of VAAL]{Illustration of the \gls{vaal} idea taken from the paper of Sinha et al. \cite{sinha2019variational}.}
    \label{fig:VAAL}
\end{figure}
...
and then present a new approach to approximate the loss function on the unobserved side. The fundamental observation and idea of \gls{alasso} can be
seen in Figure \ref{fig:Alasso}. Since \gls{alasso} extends the \gls{si} framework, we strongly recommend the reader familiarize themselves with \gls{si} before
...
\begin{figure} [ht]
    ...
    \caption[Visualization of ALASSO]{Illustration of the \gls{alasso} idea taken from the paper of Park et al. \cite{park2019continual}. The authors of \gls{alasso}
     state that, \gls{si}
...
The second approach to matching the moments is called mode-based incremental moment matching, and it is also known as mode-\gls{imm}. mode-\gls{imm}
...
$l_2$-regularization, is the next transfer technique. L2-transfer can be seen as a special form of \gls{ewc} where all the $F_i$s are set to 1. 
